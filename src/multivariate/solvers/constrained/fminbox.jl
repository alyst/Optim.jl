...
# construct an optimizer to solve the barrier problem using Fminbox.method as the reference
...
barrier_method(m::Union{NelderMead, SimulatedAnnealing, ParticleSwarm, BFGS},
...
function optimize(
    ...
    for i in eachindex(gbarrier)
        ...
        elseif thisx < thisl || thisx > thisu
            error("Initial x[$(ind2sub(x, i))]=$thisx is outside of [$thisl, $thisu] box")
        end
